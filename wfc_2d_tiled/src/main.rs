...
fn wfc() {
    ...
    if PROCESSING_TILE_SIZE < PROCESSING_TILE_OVERLAP {
        panic!("PROCESSING_TILE_SIZE must be greater than PROCESSING_TILE_OVERLAP");
    }
    ...
    let mut processing_grid = vec![EMPTY; PROCESSING_TILE_SIZE * PROCESSING_TILE_SIZE];
    let mut processing_grid_x = 0;
    let mut processing_grid_y = 0;

    let mut processing_grid_clone;
    let mut processing_grid_loop_clone;
    let mut processing_grid_loop_clone_use = 0;
    let mut processing_grid_loop_clone_reset = 0;
    let mut rng = rand::thread_rng();

    'x: while processing_grid_x <= GRID_SIZE - PROCESSING_TILE_SIZE {
        'y: while processing_grid_y <= GRID_SIZE - PROCESSING_TILE_SIZE {
            ...
            processing_grid_clone = processing_grid.clone();

            'sub: while processing_grid.iter().any(|x| x.len() > 1) {
                processing_grid_loop_clone = processing_grid.clone();
                if processing_grid_loop_clone_reset > 100 {
                    processing_grid_loop_clone_reset = 0;
                    processing_grid_y = 0;
                    break 'y;
                }
                ...
                if processing_grid_loop_clone_use > 100 {
                    ...
                    processing_grid_loop_clone_reset += 1;
                    continue;
                }
                ...
                while !stack.is_empty() {
                    ...
                    // up
                    if stack_item > PROCESSING_TILE_SIZE
                        && processing_grid[stack_item - PROCESSING_TILE_SIZE].len() > 1
                    {
                    ...
                    // left
                    if stack_item > 0 && processing_grid[stack_item - 1].len() > 1 {
                        stack.push_back(stack_item - 1);
                    }
                    // right
                    if stack_item < (PROCESSING_TILE_SIZE * PROCESSING_TILE_SIZE) - 1
                        && processing_grid[stack_item + 1].len() > 1
                    {
                        stack.push_back(stack_item + 1);
                    }
                }
            ...
            processing_grid_y += PROCESSING_TILE_SIZE - (PROCESSING_TILE_OVERLAP);
            processing_grid_loop_clone_use = 0;
            processing_grid_loop_clone_reset = 0;
        }
        processing_grid_y = 0;
        processing_grid_x += PROCESSING_TILE_SIZE - (PROCESSING_TILE_OVERLAP);
    }

    print_grid(GRID_SIZE, &grid, &tiles);
}

fn print_grid(grid_size: usize, grid: &Vec<Vec<u32>>, tiles: &HashMap<u32, Tile>) {
    let mut row_count = 1;
    let mut output = "\n".to_string();

    for cell in grid {
        let tile_char = if cell.len() == 1 {
